...
module DataMemory(Address, WriteData, Clk, MemWrite, MemRead, ReadData); 
    ...
    always @ (posedge Clk) begin
      ...
      end else if (MemWrite == 2)begin // sh
        memory[Address>>2] <= 32'b0;
        memory[Address>>2][15:0] <= WriteData [15:0];
      end else if (MemWrite == 3)begin //sb
        memory[Address>>2] <= 32'b0;
    ...
    always @ (*) begin
      if (MemRead == 2'b01) begin  // lw
            ReadData <= memory[Address>>2];
      end else if (MemRead == 2'b10) begin //lh
        ReadData <= 32'b0;
        ReadData <= memory[Address>>2][15:0];
      end else if(MemRead == 2'b11) begin // lb
        ReadData <= 32'b0;
        ReadData <= memory[Address>>2][7:0];
